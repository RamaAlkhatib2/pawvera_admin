...
class _RemindersPageState extends State<RemindersPage> {
  ...
  final types = [
    {'title': 'Vaccination', 'category': 'Health', 'desc': 'Annual or periodic vaccination reminders'},
    {'title': 'Grooming', 'category': 'Grooming', 'desc': 'Regular grooming appointments'},
    {'title': 'Vet Checkup', 'category': 'Health', 'desc': 'Routine veterinary checkups'},
    {'title': 'Medication', 'category': 'Health', 'desc': 'Medication administration reminders'},
  ];
    ...
    showDialog<void>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
          TextField(controller: titleCtl, decoration: InputDecoration(hintText: 'Title', border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)))),
          const SizedBox(height: 12),
          DropdownButtonFormField<String>(value: cat, items: const [DropdownMenuItem(value: 'Health', child: Text('Health')), DropdownMenuItem(value: 'Grooming', child: Text('Grooming'))], onChanged: (v) => cat = v ?? 'Health', decoration: const InputDecoration(border: OutlineInputBorder())),
          const SizedBox(height: 12),
          TextField(controller: descCtl, maxLines: 3, decoration: InputDecoration(hintText: 'Description', border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)))),
        ])),
        actions: [TextButton(onPressed: () => Navigator.of(ctx).pop(), child: const Text('Cancel')), ElevatedButton(onPressed: () {
          if (titleCtl.text.trim().isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please enter a title'))); return; }
          setState(() => types.insert(0, {'title': titleCtl.text.trim(), 'category': cat, 'desc': descCtl.text.trim()}));
          Navigator.of(ctx).pop();
  ...
  @override
  Widget build(BuildContext context) {
    final filtered = types.where((t) => t['title']!.toLowerCase().contains(q.toLowerCase())).toList();
    final total = types.length;
    final health = types.where((t) => (t['category'] as String).toLowerCase() == 'health').length;
    final grooming = types.where((t) => (t['category'] as String).toLowerCase() == 'grooming').length;

    return PageScaffold(
      title: 'Reminders',
      onNavigate: widget.onNavigate,
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Manage reminder types for pet care', style: TextStyle(color: Colors.grey)),
        const SizedBox(height: 16),
        ...
        Container(
          ...
          decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8)),
          child: Column(children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('Reminder Types', style: TextStyle(fontWeight: FontWeight.bold)), ElevatedButton(onPressed: _openAddDialog, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF0B233F)), child: const Text('+ Add Type'))]),
            const SizedBox(height: 12),

            // Search
            TextField(onChanged: (v) => setState(() => q = v), decoration: InputDecoration(hintText: 'Search reminder types...', prefixIcon: const Icon(Icons.search), border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)), contentPadding: const EdgeInsets.symmetric(vertical: 12))),
            const SizedBox(height: 12),
            ...
            Column(children: [for (var t in filtered)
              Container(
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Row(children: [
                    Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: const Color(0xFFF0F6F4), borderRadius: BorderRadius.circular(8)), child: const Icon(Icons.notifications)),
                    const SizedBox(width: 12),
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Text(t['title']!, style: const TextStyle(fontWeight: FontWeight.bold)), const SizedBox(width: 8), Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: const Color(0xFFF3F4F6), borderRadius: BorderRadius.circular(8)), child: Text(t['category']!, style: const TextStyle(fontSize: 12)))]), const SizedBox(height: 6), Text(t['desc']!, style: TextStyle(color: Colors.grey.shade700))])
                  ]),
                  Row(children: [IconButton(onPressed: () {}, icon: const Icon(Icons.edit)), IconButton(onPressed: () { setState(() => types.remove(t)); }, icon: const Icon(Icons.delete, color: Colors.red))])
                ]),
              )
            ])
          ]),
        ...
        // Stats cards
        Row(children: [Expanded(child: Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Total Reminder Types', style: TextStyle(color: Colors.grey)), const SizedBox(height: 8), Text('$total', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold))]))), const SizedBox(width: 12), Expanded(child: Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Health Reminders', style: TextStyle(color: Colors.grey)), const SizedBox(height: 8), Text('$health', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold))]))), const SizedBox(width: 12), Expanded(child: Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Grooming Reminders', style: TextStyle(color: Colors.grey)), const SizedBox(height: 8), Text('$grooming', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold))])))]),

        const SizedBox(height: 12),
        Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: const Color(0xFFEFFAF4), borderRadius: BorderRadius.circular(8), border: Border.all(color: const Color(0xFFD9F0E7))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: const [Text('About Reminders', style: TextStyle(fontWeight: FontWeight.bold)), SizedBox(height: 8), Text('Reminder types help pet owners stay on top of their pet\'s care schedule. Each reminder type can be customized with specific intervals and notifications to ensure pets receive timely care.', style: TextStyle(color: Colors.grey))])),
      ]),
    );
  }
}
...
class _QrTagsPageState extends State<QrTagsPage> {
  ...
  final tags = [
    {'id': 'QR-2025-001', 'pet': 'Max', 'owner': 'Sarah Johnson', 'date': '1/5/2025', 'scans': '24'},
    {'id': 'QR-2025-002', 'pet': 'Luna', 'owner': 'Mike Chen', 'date': '1/3/2025', 'scans': '15'},
    {'id': 'QR-2024-158', 'pet': 'Charlie', 'owner': 'Emma Davis', 'date': '12/20/2024', 'scans': '8'},
    {'id': 'QR-2025-003', 'pet': 'Bella', 'owner': 'John Smith', 'date': '1/2/2025', 'scans': '32'},
    {'id': 'QR-2024-145', 'pet': 'Rocky', 'owner': 'Lisa Brown', 'date': '12/15/2024', 'scans': '5'},
  ];

  int get totalScans => tags.fold<int>(0, (s, t) => s + (int.tryParse(t['scans']!) ?? 0));
  ...
  Widget build(BuildContext context) {
    final qLower = q.toLowerCase();
    final filtered = tags.where((t) => t['id']!.toLowerCase().contains(qLower) || t['pet']!.toLowerCase().contains(qLower) || t['owner']!.toLowerCase().contains(qLower)).toList();
    ...
    Widget _statCard(String label, String value, Color color, IconData icon) {
      return Container(
        ...
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8)),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: color.withOpacity(0.12), borderRadius: BorderRadius.circular(8)), child: Icon(icon, color: color)), const SizedBox(width: 12), Text(label, style: const TextStyle(color: Colors.grey))]),
          const SizedBox(height: 12),
    ...
    return PageScaffold(
      ...
      onNavigate: widget.onNavigate,
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Monitor QR tag usage and scan analytics across the platform.', style: TextStyle(color: Colors.grey)),
        const SizedBox(height: 16),
        ...
        GridView.count(
          ...
          physics: const NeverScrollableScrollPhysics(),
          children: [
            _statCard('Total Tags', tags.length.toString(), const Color(0xFF9B7BD7), Icons.qr_code),
            _statCard('Total Scans', totalScans.toString(), const Color(0xFF6FB4FF), Icons.bar_chart),
            _statCard('Avg Scans/Tag', avgScans.toStringAsFixed(0), const Color(0xFF7EE9D1), Icons.show_chart),
          ],
        ),

        const SizedBox(height: 12),

        TextField(onChanged: (v) => setState(() => q = v), decoration: InputDecoration(hintText: 'Search by pet name, owner, or QR code...', prefixIcon: const Icon(Icons.search), border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)), contentPadding: const EdgeInsets.symmetric(vertical: 12))),
        ...
        Container(
          ...
          decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8)),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('All QR Tags', style: TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 12),
            ...
            Padding(
              ...
              child: Row(children: [
                ...
                Expanded(flex: 2, child: Text('Tag ID', style: TextStyle(color: Colors.grey.shade600))),
                Expanded(flex: 2, child: Text('Pet Name', style: TextStyle(color: Colors.grey.shade600))),
                Expanded(flex: 2, child: Text('Owner', style: TextStyle(color: Colors.grey.shade600))),
                Expanded(flex: 2, child: Text('Created Date', style: TextStyle(color: Colors.grey.shade600))),
                Expanded(flex: 1, child: Align(alignment: Alignment.centerRight, child: Text('Total Scans', style: TextStyle(color: Colors.grey.shade600)))),
              ]),
            ),
            ...
            Column(children: [for (var t in filtered)
              Padding(
                ...
                child: Row(children: [
                  Container(padding: const EdgeInsets.all(6), decoration: BoxDecoration(color: const Color(0xFFF4E7FF), borderRadius: BorderRadius.circular(6)), child: const Icon(Icons.qr_code, color: Color(0xFF8F56D6))),
                  const SizedBox(width: 12),
                  Expanded(flex: 2, child: Text(t['id']!, style: const TextStyle(fontWeight: FontWeight.bold))),
                  Expanded(flex: 2, child: Text(t['pet']!, style: TextStyle(color: Colors.grey.shade800))),
                  Expanded(flex: 2, child: Text(t['owner']!, style: TextStyle(color: Colors.grey.shade700))),
                  Expanded(flex: 2, child: Text(t['date']!, style: TextStyle(color: Colors.grey.shade700))),
                  Expanded(flex: 1, child: Align(alignment: Alignment.centerRight, child: Text(t['scans']!, style: const TextStyle(fontWeight: FontWeight.bold)))),
                ]),
              )
            ])
          ]),
...
class _AuditLogsPageState extends State<AuditLogsPage> {
  ...
  final logs = <Map<String, String>>[
    {'time': '2025-01-07 14:32:15', 'admin': 'Admin User', 'action': 'User Login', 'category': 'auth', 'details': 'Successful admin login', 'ip': '192.168.1.100', 'status': 'success'},
    {'time': '2025-01-07 14:28:42', 'admin': 'Sarah Mitchell', 'action': 'Updated User Profile', 'category': 'user', 'details': 'Modified user profile for Sarah Johnson (ID: 12458)', 'ip': '192.168.1.105', 'status': 'success'},
    {'time': '2025-01-07 14:15:30', 'admin': 'Admin User', 'action': 'Deleted Pet Record', 'category': 'data', 'details': 'Removed pet record: Max (ID: 3421)', 'ip': '192.168.1.100', 'status': 'warning'},
    {'time': '2025-01-07 13:58:12', 'admin': 'John Parker', 'action': 'Changed Settings', 'category': 'settings', 'details': 'Updated email notification preferences', 'ip': '192.168.1.112', 'status': 'success'},
    {'time': '2025-01-07 13:45:20', 'admin': 'Admin User', 'action': 'Failed Login Attempt', 'category': 'auth', 'details': 'Invalid password entered', 'ip': '192.168.1.100', 'status': 'failed'},
    {'time': '2025-01-07 13:30:05', 'admin': 'Sarah Mitchell', 'action': 'Created Store Owner', 'category': 'user', 'details': 'Added new store owner: Pet Essentials Plus', 'ip': '192.168.1.105', 'status': 'success'},
    {'time': '2025-01-07 12:22:48', 'admin': 'John Parker', 'action': 'Database Backup', 'category': 'data', 'details': 'Initiated manual database backup', 'ip': '192.168.1.112', 'status': 'success'},
    {'time': '2025-01-07 11:55:33', 'admin': 'Admin User', 'action': 'Security Update', 'category': 'security', 'details': 'Enabled two-factor authentication', 'ip': '192.168.1.100', 'status': 'success'},
    {'time': '2025-01-07 11:20:17', 'admin': 'Sarah Mitchell', 'action': 'Approved Order', 'category': 'data', 'details': 'Approved order ORD-2025-003 for Emma Davis', 'ip': '192.168.1.105', 'status': 'success'},
    {'time': '2025-01-07 10:45:59', 'admin': 'John Parker', 'action': 'Updated Vet Clinic', 'category': 'user', 'details': 'Modified clinic details for Central Vet Hospital', 'ip': '192.168.1.112', 'status': 'success'},
  ];
  ...
  Widget build(BuildContext context) {
    final qLower = q.toLowerCase();
    final filtered = logs.where((l) => l['admin']!.toLowerCase().contains(qLower) || l['action']!.toLowerCase().contains(qLower) || l['details']!.toLowerCase().contains(qLower)).toList();

    Widget _statCard(String label, String value, IconData icon, Color color) {
      return Container(
        padding: const EdgeInsets.symmetric(vertical: 18, horizontal: 16),
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8)),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: color.withOpacity(0.12), borderRadius: BorderRadius.circular(8)), child: Icon(icon, color: color)), const SizedBox(width: 12), Text(label, style: const TextStyle(color: Colors.grey))]), const SizedBox(height: 12), Text(value, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold))]),
      );
    }
    ...
    return PageScaffold(
      ...
      onNavigate: widget.onNavigate,
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Track all administrative actions and system events', style: TextStyle(color: Colors.grey)),
        const SizedBox(height: 16),

        GridView.count(crossAxisCount: 4, shrinkWrap: true, crossAxisSpacing: 12, mainAxisSpacing: 12, childAspectRatio: 2.4, physics: const NeverScrollableScrollPhysics(), children: [
          _statCard('Total Logs', totalLogs.toString(), Icons.show_chart, const Color(0xFF6FB4FF)),
          _statCard('Successful Actions', successCount.toString(), Icons.check_circle, const Color(0xFF7EE9D1)),
          _statCard('Failed Actions', failedCount.toString(), Icons.error, const Color(0xFFFFC6C6)),
          _statCard('Warnings', warningCount.toString(), Icons.warning, const Color(0xFFFFE6B3)),
        ]),
        ...
        Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8)), child: Row(children: [
          Expanded(child: TextField(onChanged: (v) => setState(() => q = v), decoration: InputDecoration(hintText: 'Search by admin, action, or details...', prefixIcon: const Icon(Icons.search), border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)), contentPadding: const EdgeInsets.symmetric(vertical: 12)))),
          const SizedBox(width: 12),
          Container(padding: const EdgeInsets.symmetric(horizontal: 12), decoration: BoxDecoration(border: Border.all(color: Colors.grey.shade300), borderRadius: BorderRadius.circular(8)), child: DropdownButton<String>(value: categoryFilter, underline: const SizedBox.shrink(), items: const [DropdownMenuItem(value: 'All Categories', child: Text('All Categories')), DropdownMenuItem(value: 'auth', child: Text('Auth')), DropdownMenuItem(value: 'user', child: Text('User')), DropdownMenuItem(value: 'data', child: Text('Data'))], onChanged: (v) => setState(() => categoryFilter = v ?? 'All Categories'))),
          const SizedBox(width: 12),
          Container(padding: const EdgeInsets.symmetric(horizontal: 12), decoration: BoxDecoration(border: Border.all(color: Colors.grey.shade300), borderRadius: BorderRadius.circular(8)), child: DropdownButton<String>(value: statusFilter, underline: const SizedBox.shrink(), items: const [DropdownMenuItem(value: 'All Status', child: Text('All Status')), DropdownMenuItem(value: 'success', child: Text('Success')), DropdownMenuItem(value: 'failed', child: Text('Failed')), DropdownMenuItem(value: 'warning', child: Text('Warning'))], onChanged: (v) => setState(() => statusFilter = v ?? 'All Status'))),
          const SizedBox(width: 12),
          ElevatedButton(onPressed: () {}, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF0B67C1)), child: const Row(children: [Icon(Icons.filter_list), SizedBox(width: 8), Text('Filter')]))
        ])),

        const SizedBox(height: 16),

        // Table container
        Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          // table header
          Padding(padding: const EdgeInsets.symmetric(vertical: 8.0), child: Row(children: [Expanded(flex: 2, child: Text('Timestamp', style: TextStyle(color: Colors.grey.shade600))), Expanded(flex: 2, child: Text('Admin', style: TextStyle(color: Colors.grey.shade600))), Expanded(flex: 2, child: Text('Action', style: TextStyle(color: Colors.grey.shade600))), Expanded(flex: 2, child: Text('Category', style: TextStyle(color: Colors.grey.shade600))), Expanded(flex: 4, child: Text('Details', style: TextStyle(color: Colors.grey.shade600))), Expanded(flex: 2, child: Text('IP Address', style: TextStyle(color: Colors.grey.shade600))), Expanded(flex: 1, child: Align(alignment: Alignment.centerRight, child: Text('Status', style: TextStyle(color: Colors.grey.shade600))))) ]),

          const SizedBox(height: 8),

          Column(children: [for (var l in filtered)
            Padding(padding: const EdgeInsets.symmetric(vertical: 10.0), child: Row(children: [
              Expanded(flex: 2, child: Text(l['time']!, style: TextStyle(color: Colors.grey.shade700))),
              Expanded(flex: 2, child: Row(children: [CircleAvatar(radius: 14, backgroundColor: Colors.deepPurple.shade200, child: const Icon(Icons.person, color: Colors.white, size: 16)), const SizedBox(width: 8), Text(l['admin']!, style: const TextStyle(fontWeight: FontWeight.bold))])),
              Expanded(flex: 2, child: Text(l['action']!, style: const TextStyle(fontWeight: FontWeight.bold))),
              Expanded(flex: 2, child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 6), decoration: BoxDecoration(color: const Color(0xFFF3F4F6), borderRadius: BorderRadius.circular(12)), child: Text(l['category']!, style: const TextStyle(fontSize: 12)))),
              Expanded(flex: 4, child: Text(l['details']!, style: TextStyle(color: Colors.grey.shade700))),
              Expanded(flex: 2, child: Text(l['ip']!, style: TextStyle(color: Colors.grey.shade700))),
              Expanded(flex: 1, child: Align(alignment: Alignment.centerRight, child: Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(color: _statusColor(l['status']!), borderRadius: BorderRadius.circular(16)), child: Text(l['status']!.toUpperCase(), style: TextStyle(color: _statusTextColor(l['status']!), fontSize: 12)))))),
          ])
        ])),
      ]),
    );
